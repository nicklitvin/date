...
import { getImageDetails, getImageInput } from "../__testUtils__/easySetup";
...
describe("image", () => {
    ...
    it("should upload image", async () => {
        expect(await funcs.uploadImage(await getImageInput(true))).not.toEqual(null);
    })
    ...
    it("should get imageURL", async () => {
        const imageID = await funcs.uploadImage(await getImageInput(true)) as string;
    ...
    it("should get all imageIDs", async () => {
        const image = await getImageInput(true);
        await Promise.all([
